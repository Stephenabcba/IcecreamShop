	...
	<div class="container">
		<div class="grid all-icecreams-grid">
			<c:forEach var="flavor" items="${flavors }">
				<a href="/icecreams/${flavor.id }" draggable="false"><div class="icecream-card border">
					<img src="${flavor.imageUrl }" alt="<c:out value="${flavor.name }"/>-icecream" draggable="false"/>
					<div class="card-body"><c:out value="${flavor.name }"/></div>
				</div></a>
			</c:forEach>
		</div>
	</div>
